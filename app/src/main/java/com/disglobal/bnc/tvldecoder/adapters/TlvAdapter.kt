package com.disglobal.bnc.tvldecoder.adapters

import com.disglobal.bnc.tvldecoder.store.EMVTagStore
import com.disglobal.bnc.tvldecoder.utils.HexConversion
import com.disglobal.bnc.tvldecoder.utils.TlvDecoder

fun main() {
    // Cadena de texto de ejemplo
    val hexString =
        ""
    val tagMeaning = EMVTagStore.emvMap.get(hexString.substring(0, 4))?.name
    //when
    val tlvList = TlvDecoder.parseString(tagMeaning ?: "")

    // Mostrar todos los tags y valores en formato de tabla
    println("| TAG   |           TAG name              | ValueHex                                                                        |  ValueExt |")
    println("|--     |   --                            |--                                                                               | --        |")
    for (tlv in tlvList) {
        val valueExt =
            if (tlv.tag == "DF834F" || tlv.tag == "DF8344" || tlv.tag == "DF834C" || tlv.tag == "DF834B") {
                "(${HexConversion.ToAscii(tlv.value)})"
            } else {
                "null"
            }
        println("| ${tlv.tag} | ${tlv.tagMeaning} | ${tlv.value} | $valueExt |")
    }
}

