...
open class EmvActivity2 : ComponentActivity(), OnCardInfoListener, OnEmvProcessListener2,
    ...
    fun onCreate(savedInstanceState: Bundle?) {
        ...
        mContext = this
        deviceEngine = (this.applicationContext as nexgobnc).deviceEngine
        ...
        pwdAlertDialog = Builder(this).setView(dv).create()
        ...
        emvUtils = EmvUtils(this@EmvActivity2).aidList
    }
    ...
    private fun initEmvAid() {
        ...
        if (emvHandler2!!.aidListNum <= 0) {
            val aidEntityList: List<AidEntity> = emvUtils.aidList
    ...
    private fun initEmvCapk() {
        ...
        if (capk_num <= 0) {
            val capkEntityList: List<CapkEntity> = emvUtils.getCapkList()
    ...
    private fun inputAmount() {
        runOnUiThread(Runnable {
            ...
            Builder(mContext).setTitle("Input Amount")
                .setView(et)
...
//            emvHandler2.contactlessSetAidFirstSelect((byte) 0x07, ByteUtils.hexString2ByteArray("a0000001524010"));
    ...
    override fun onSelApp(
        ...
        runOnUiThread(Runnable {
            val dv: View = getLayoutInflater().inflate(R.layout.dialog_app_list, null)
            val alertDialog: AlertDialog = Builder(mContext).setView(dv).create()
...
//        emvHandler.onSetReadCardAgainResponse(true);
    ...
    override fun onCardHolderInputPin(isOnlinePin: Boolean, leftTimes: Int) {
        ...
        runOnUiThread(Runnable { showInputPin(isOnlinePin) })
    }
