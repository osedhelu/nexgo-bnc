...
class TlvBuilder {
    companion object {
        ...
        val TAG_PAN = byteArrayOf(0x5A)                                // Application Primary Account Number (PAN)
        val TAG_TVR = byteArrayOf(0x95.toByte())                                // Terminal Verification Results
        val TAG_TRANSACTION_DATE = byteArrayOf(0x9A.toByte())                   // Transaction Date
        val TAG_TSI = byteArrayOf(0x9B.toByte())                                // Transaction Status Information
        ...
        val TAG_APP_EXPIRATION_DATE = byteArrayOf(0x5F, 0x24)          // Application Expiration Date
        ...
        val TAG_AMOUNT_AUTHORIZED = byteArrayOf(0x9F.toByte(), 0x02)            // Amount, Authorized
        val TAG_MERCHANT_ID = byteArrayOf(0x9F.toByte(), 0x16)                  // Merchant Identifier
        val TAG_TERMINAL_COUNTRY_CODE = byteArrayOf(0x9F.toByte(), 0x1A)        // Terminal Country Code
        val TAG_TERMINAL_ID = byteArrayOf(0x9F.toByte(), 0x1C)                  // Terminal Identification
        val TAG_INTERFACE_DEVICE_SERIAL = byteArrayOf(0x9F.toByte(), 0x1E)      // Interface Device Serial Number
        val TAG_TRANSACTION_TIME = byteArrayOf(0x9F.toByte(), 0x21)             // Transaction Time
        val TAG_TERMINAL_CAPABILITIES = byteArrayOf(0x9F.toByte(), 0x33)        // Terminal Capabilities
        val TAG_TERMINAL_TYPE = byteArrayOf(0x9F.toByte(), 0x35)                // Terminal Type
        val TAG_ATC = byteArrayOf(0x9F.toByte(), 0x36)                          // Application Transaction Counter
        val TAG_UNPREDICTABLE_NUMBER = byteArrayOf(0x9F.toByte(), 0x37)         // Unpredictable Number
        val TAG_POS_ENTRY_MODE = byteArrayOf(0x9F.toByte(), 0x39)               // Point-of-Service Entry Mode
        val TAG_ADDITIONAL_TERMINAL_CAPABILITIES = byteArrayOf(0x9F.toByte(), 0x40) // Additional Terminal Capabilities
        val TAG_TRANSACTION_SEQUENCE_COUNTER = byteArrayOf(0x9F.toByte(), 0x41) // Transaction Sequence Counter
        val TAG_MERCHANT_NAME_LOCATION = byteArrayOf(0x9F.toByte(), 0x4E)       // Merchant Name and Location
        
        // Tags específicos para criptogramas
        val TAG_APPLICATION_CRYPTOGRAM = byteArrayOf(0x9F.toByte(), 0x26)       // Application Cryptogram
        val TAG_CRYPTOGRAM_INFO_DATA = byteArrayOf(0x9F.toByte(), 0x27)         // Cryptogram Information Data
        val TAG_ISSUER_APP_DATA = byteArrayOf(0x9F.toByte(), 0x10)              // Issuer Application Data
        val TAG_AIP = byteArrayOf(0x82.toByte())                                // Application Interchange Profile
        val TAG_AID = byteArrayOf(0x84.toByte())                                // Dedicated File Name (AID)
        ...
        val TAG_MESSAGE_TYPE = byteArrayOf(0xDF.toByte(), 0x83.toByte(), 0x30.toByte()) // Message Type
        val TAG_RETRIEVAL_REFERENCE_NUMBER = byteArrayOf(0xDF.toByte(), 0x83.toByte(), 0x31.toByte()) // Retrieval Reference Number
        val TAG_BATCH_NUMBER = byteArrayOf(0xDF.toByte(), 0x83.toByte(), 0x32.toByte()) // Batch Number
        val TAG_STAN = byteArrayOf(0xDF.toByte(), 0x83.toByte(), 0x33.toByte()) // System Trace Audit Number
        val TAG_INVOICE_NUMBER = byteArrayOf(0xDF.toByte(), 0x83.toByte(), 0x34.toByte()) // Invoice Number
        ...
        fun buildSaleTlvMessage(
            ...
            tlvMap[ByteUtils.byteArray2HexString(TAG_AMOUNT_AUTHORIZED)] = ByteUtils.byteArray2HexString(amountBytes)
        ...
        private fun addEmvDataToTlvMap(emvHandler2: EmvHandler2, tlvMap: HashMap<String, String>) {
            ...
            for (tag in emvTags) {
                ...
                if (value != null) {
                    tlvMap[ByteUtils.byteArray2HexString(tag)] = ByteUtils.byteArray2HexString(value)
                }
            }
        }
