...
class EmvCardReaderNew(
    private val deviceEngine: DeviceEngine,
    private val cardReader: CardReader,
    private val emvHandler: EmvHandler2,
) : OnCardInfoListener, OnEmvProcessListener2, OnPinPadInputListener {
    private var job: Job? = null
    private val _passwordPIN = MutableStateFlow("")
    val passwordPIN = _passwordPIN.asStateFlow()
    ...
    fun checkAidDetail() {
        job = CoroutineScope(Dispatchers.Default).launch {
            withContext(Dispatchers.IO) {
                val aidEntities = emvHandler?.aidList
                if (aidEntities == null) {
                ...
                val capkEntities = emvHandler?.capkList
...
// Métodos para responder a eventos de la UI
    ...
    fun onCardNumberConfirmed(confirmed: Boolean) {
        emvHandler?.onSetConfirmCardNoResponse(confirmed)
    }
...
// Implementaciones de los listeners del SDK
    ...
    private fun startEmvTest() {
        ...
        if (deviceEngine == null) {
            ...
            _transactionState.value = TransactionState.Error("Motor del dispositivo no disponible")
        ...
        if (cardReader == null) {
            ...
            _transactionState.value = TransactionState.Error("Lector de tarjetas no disponible")
        ...
        _transactionState.value = TransactionState.CardReading
        ...
        if (searchResult != SdkResult.Success) {
            ...
            _transactionState.value = TransactionState.Error(errorMessage)
    ...
    override fun onCardInfo(retCode: Int, cardInfo: CardInfoEntity?) {
        ...
        if (retCode == SdkResult.Success && cardInfo != null) {
            ...
            // Iniciar proceso EMV
            _transactionState.value = TransactionState.ProcessingEmv
    ...
    override fun onSwipeIncorrect() {
        _transactionState.value =
            TransactionState.Error("Deslizamiento incorrecto, intente nuevamente")
        emvProcessListener?.onRequestShowToast("Deslizamiento incorrecto, intente nuevamente")
    }

    override fun onMultipleCards() {
        _transactionState.value =
            TransactionState.Error("Se detectaron múltiples tarjetas, use solo una")
    ...
    override fun onSelApp(
        ...
        _transactionState.value = TransactionState.SelectApplication(appNameList)
    ...
    override fun onConfirmCardNo(cardInfo: CardInfoEntity) {
        ...
        _transactionState.value = TransactionState.ConfirmCardNumber(cardNo ?: "")
    ...
    override fun onCardHolderInputPin(isOnlinePin: Boolean, leftTimes: Int) {
        ...
        _transactionState.value = TransactionState.PinRequested
    ...
    override fun onOnlineProc() {
        ...
        _transactionState.value = TransactionState.ProcessingOnline

        // Obtener datos importantes para la transacción
        Log.d("nexgo", "getEmvContactlessMode:" + emvHandler?.emvContactlessMode)
        Log.d("nexgo", "getcardinfo:" + Gson().toJson(emvHandler?.emvCardDataInfo))
        Log.d("nexgo", "getEmvCvmResult:" + emvHandler?.emvCvmResult)
        Log.d("nexgo", "getSignNeed--" + emvHandler?.signNeed)

        // Obtener TLVs importantes para enviar al servidor
        val tlv_5A = emvHandler?.getTlv(byteArrayOf(0x5A.toByte()), EmvDataSourceEnum.FROM_KERNEL)
        Log.d("nexgo", "PAN (5A)--" + (tlv_5A?.let { ByteUtils.byteArray2HexString(it) } ?: "null"))

        val tlv_95 = emvHandler?.getTlv(byteArrayOf(0x95.toByte()), EmvDataSourceEnum.FROM_KERNEL)
        Log.d("nexgo", "TVR (95)--" + (tlv_95?.let { ByteUtils.byteArray2HexString(it) } ?: "null"))

        val tlv_84 = emvHandler?.getTlv(byteArrayOf(0x84.toByte()), EmvDataSourceEnum.FROM_KERNEL)
        Log.d("nexgo", "AID (84)--" + (tlv_84?.let { ByteUtils.byteArray2HexString(it) } ?: "null"))

        val tlv_50 = emvHandler?.getTlv(byteArrayOf(0x50.toByte()), EmvDataSourceEnum.FROM_KERNEL)
        Log.d("nexgo",
            "Application Label (50)--" + (tlv_50?.let { ByteUtils.byteArray2HexString(it) }
                ?: "null"))

        val tlv_9F26 = emvHandler?.getTlv(
            byteArrayOf(0x9F.toByte(), 0x26.toByte()), EmvDataSourceEnum.FROM_KERNEL
        )
        Log.d("nexgo",
            "Application Cryptogram (9F26)--" + (tlv_9F26?.let { ByteUtils.byteArray2HexString(it) }
                ?: "null"))

        val tlv_9F27 = emvHandler?.getTlv(
            byteArrayOf(0x9F.toByte(), 0x27.toByte()), EmvDataSourceEnum.FROM_KERNEL
        )
        Log.d("nexgo", "Cryptogram Information Data (9F27)--" + (tlv_9F27?.let {
            ByteUtils.byteArray2HexString(it)
        } ?: "null"))

        val tlv_9F36 = emvHandler?.getTlv(
            byteArrayOf(0x9F.toByte(), 0x36.toByte()), EmvDataSourceEnum.FROM_KERNEL
        )
        Log.d("nexgo", "Application Transaction Counter (9F36)--" + (tlv_9F36?.let {
            ByteUtils.byteArray2HexString(it)
        } ?: "null"))
    ...
    private fun buildField55(): String {
        ...
        for (tag in commonTags) {
            val value = emvHandler?.getTlv(tag, EmvDataSourceEnum.FROM_KERNEL)
            if (value != null) {
    ...
    override fun onFinish(retCode: Int, entity: EmvProcessResultEntity?) {
        ...
        Log.d("nexgo", "emvHandler.getSignNeed()--" + emvHandler?.signNeed)
        Log.d("nexgo", "getcardinfo:" + Gson().toJson(emvHandler?.emvCardDataInfo))
        Log.d("nexgo", "getEmvCvmResult:" + emvHandler?.emvCvmResult)

        // Notificar a la UI
        _transactionState.value = TransactionState.TransactionCompleted(retCode)
...
// Métodos auxiliares
    ...
    private fun showInputPin(isOnlinePin: Boolean) {
        ...
        if (isOnlinePin) {
            if (cardNo == null) {
                cardNo = emvHandler?.emvCardDataInfo?.cardNo
            }
